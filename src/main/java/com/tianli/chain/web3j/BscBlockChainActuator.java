...
public class BscBlockChainActuator {
    ...
    /**
     * 代币的 合约调用方法 转移资产用，需大额gaslimit
     */
    public Result tokenSendRawTransaction1(long nonce, String curContractAddress, String chainParams, String password, String operation) {
        String gas_limit = configService.getOrDefault(ConfigConstants.BSC_GAS_LIMIT_PLUS, "10000000");
        String price = bnbGas();
        if(StringUtils.isBlank(price)){
            price = "8";
        }
        return tokenSendRawTransaction(nonce, curContractAddress, chainParams, price, gas_limit, password, BigInteger.ZERO, operation);
    }
    ...
    public <T extends Type> Result ethCall(String contractName, String methodName, List<Type> params, Class<T> result) throws IOException {
        ...
        EthCall send = bscWeb3j.ethCall(Transaction.createEthCallTransaction(null, curContractAddress,
    ...
    public EthGasAPIResponse ethGas() {
        ...
        if (safeGasPrice != null && proposeGasPrice != null && fastGasPrice != null) {
            ...
            ethgasAPIResponse.setSafeLow(safeGasPrice * 10.0);
        }
    ...
    public String bnbGas() {
        ...
        String stringResult = null;
        try {
            HttpResponse httpResponse = HttpUtils.doGet("https://api.bscscan.com/api?module=proxy&action=eth_gasPrice", "", "", Map.of(), Map.of());
            stringResult = EntityUtils.toString(httpResponse.getEntity());
        } catch (Exception ignore) {
        }
        JsonObject jsonObject = new Gson().fromJson(stringResult, JsonObject.class);
        ...
        try {
            String result = jsonObject.get("result").getAsString().substring(2);
            bigInteger = new BigInteger(result, 16);
        } catch (Exception ignored) {
            return null;
        }
    ...
    public String getSymbol(String address) throws IOException {
        String value = getEthCallWithParam(bscWeb3j, address, "symbol", List.of()).getValue();
    ...
    public String getName(String address) throws IOException {
        String value = getEthCallWithParam(bscWeb3j, address, "name", List.of()).getValue();
        List<Type> list = FunctionReturnDecoder.decode(value,
                List.of(TypeReference.create((Class) Utf8String.class))
        );
    ...
    public int getDecimal(String address) throws IOException {
        String value = getEthCallWithParam(bscWeb3j, address, "decimals", List.of()).getValue();
        List<Type> list = FunctionReturnDecoder.decode(value,
